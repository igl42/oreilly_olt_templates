...
class FixedVector final
{
 ...
 public:
   ...
   void push_back( Type&& value )
   {
      ...
      construct_at( end(), std::move( value ) );
      ++size_;
...
{
   Test() { std::puts( "Test()" ); }
   Test( int j ) : i( j ) { std::puts( "Test(int)" ); }
   ~Test() { std::puts( "~Test()" ); }
   Test( Test const& t ) { i = t.i; std::puts( "Test(Test const&)" ); }
   Test( Test&& t ) { i = t.i; std::puts( "Test(Test&&)" ); }
   Test& operator=( Test const& t ) { i = t.i; std::puts( "Test& operator=(Test const&)" ); return *this; }
   Test& operator=( Test&& t ) { i = t.i; std::puts( "Test& operator=(Test&&)" ); return *this; }
