...
concept SequenceContainer =
   std::is_class_v<T> &&        // Type Trait
   std::regular<T> &&           // Concept
   requires {                   // Requires Expression
      typename T::value_type;   // 'typename' is required to ask for a type instead of a value
      typename T::iterator;
      typename T::const_iterator;
   } &&
   requires ( T cont ) {        // Requires Expression
   ...
   requires ( T const cont ) {  // Requires Expression
   ...
   requires ( T t ) {
      t == t;  // No need to introduce two instances
      t < t;
   };
...
struct IsSequenceContainer
   : public std::bool_constant< SequenceContainer<T> >
{};
